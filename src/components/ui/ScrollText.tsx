...
// Dark variant for immersive sections - cream text on dark background
...
export const DarkScrollReveal = ({ 
  ...
  return (
    <div ref={containerRef}>
      <p className={cn(
        "font-display text-3xl md:text-5xl lg:text-6xl text-center leading-tight max-w-4xl",
        className
      )}>
        {words.map((word, i) => (
          <DarkScrollWord 
...
interface DarkScrollWordProps extends WordProps {
  isHighlight?: boolean;
}

const DarkScrollWord = ({ word, index, total, scrollYProgress, isHighlight }: DarkScrollWordProps) => {
  const start = index / total;
  const end = start + (1 / total) * 2.5;
  ...
  return (
    <motion.span 
      ...
      style={{ 
        opacity, 
        y,
        color: isHighlight ? "hsl(45, 70%, 75%)" : "hsl(45, 70%, 91%)"
      }}
    >
      {word}
    </motion.span>
